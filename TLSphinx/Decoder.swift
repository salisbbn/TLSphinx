...
open class Decoder {
    ...
    fileprivate func get_hyp() -> Hypothesis? {
        var score: CInt = 0
        guard let string: UnsafePointer<CChar> = ps_get_hyp(psDecoder, &score) else{
            return nil;
        }
...
//
//        mixer.installTap(onBus: 0, bufferSize: 4096, format: formatIn, block: { (buffer: AVAudioPCMBuffer!, time: AVAudioTime!) -> Void in
//
...
//____________________________________________________________________________________________________________________________________________________________
        ...
        let ratio: Double = formatIn.sampleRate/formatOut.sampleRate
